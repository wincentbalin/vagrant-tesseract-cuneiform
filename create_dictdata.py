...
def create_freq_wordlist(cnt: collections.Counter, threshold=0.95) -> list:
    ...
    n = 0
    freq_wordlist = []
    for item in create_wordlist(cnt):
...
def write_file(filename: str, wordlist: list):
...
def main():
    ...
    write_file(os.path.join(args.directory, '{}.{}'.format(args.language, 'wordlist')), create_wordlist(wordcount))
    write_file(os.path.join(args.directory, '{}.{}'.format(args.language, 'word.bigrams')), create_bigramlist(wordbigramcount))
    write_file(os.path.join(args.directory, '{}.{}'.format(args.language, 'training_text.bigram_freqs')), create_wordlist(bigramcount))
    write_file(os.path.join(args.directory, '{}.{}'.format(args.language, 'training_text.unigram_freqs')), create_wordlist(unigramcount))
