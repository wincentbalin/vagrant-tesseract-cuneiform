...
VALID_LANGUAGE_CODES="afr akk amh ara asm aze aze_cyrl bel ben bih bod bos bul
                      cat ceb ces chi_sim chi_tra chr cym cyr_lid dan deu div
                      dzo ell eng enm epo est eus fas fil fin fra frk frm gle
                      glg grc guj hat heb hin hrv hun hye iku ind isl ita
                      ita_old jav jpn kan kat kat_old kaz khm kir kor kur lao
                      lat lat_lid lav lit mal mar mkd mlt msa mya nep nld nor
                      ori pan pol por pus ron rus san sin slk slv snd spa
                      spa_old sqi srp srp_latn swa swe syr tam tel tgk tgl tha
                      tir tur uig ukr urd uzb uzb_cyrl vie yid gle_uncial"
...
set_lang_specific_parameters() {
  ...
  case ${lang} in
    ...
    akk ) test -z "$FONTS" && FONTS=( "${CUNEIFORM_FONTS[@]}" ) ;;
